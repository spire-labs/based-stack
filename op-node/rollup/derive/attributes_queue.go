...
type AttributesBuilder interface {
	PreparePayloadAttributes(ctx context.Context, l2Parent eth.L2BlockRef, epoch eth.BlockID, electionWinners []*eth.ElectionWinner) (attrs *eth.PayloadAttributes, err error)
}
...
func (aq *AttributesQueue) NextAttributes(ctx context.Context, parent eth.L2BlockRef, electionWinners []*eth.ElectionWinner) (*AttributesWithParent, error) {
	...
	if attrs, err := aq.createNextAttributes(ctx, aq.batch, parent, electionWinners); err != nil {
		return nil, err
	} else {
...
func (aq *AttributesQueue) createNextAttributes(ctx context.Context, batch *SingularBatch, l2SafeHead eth.L2BlockRef, electionWinners []*eth.ElectionWinner) (*eth.PayloadAttributes, error) {
	...
	defer cancel()
	attrs, err := aq.builder.PreparePayloadAttributes(fetchCtx, l2SafeHead, batch.Epoch(), electionWinners)
	if err != nil {
		return nil, err
	}
