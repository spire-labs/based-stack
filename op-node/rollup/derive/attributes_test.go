...
func TestPreparePayloadAttributes(t *testing.T) {
	...
	t.Run("inconsistent next height origin", func(t *testing.T) {
		...
		attrBuilder := NewFetchingAttributesBuilder(cfg, l1Fetcher, l1CfgFetcher, emptyElectionClient)
	...
	t.Run("same origin again", func(t *testing.T) {
		...
		epoch := l1Info.ID()
		l1InfoTx, err := L1InfoDepositBytes(cfg, testSysCfg, l2Parent.SequenceNumber+1, l1Info, 0, common.Address{})
		require.NoError(t, err)
	...
	t.Run("burn tx with no deposits complete", func(t *testing.T) {
		...
		l1Fetcher.ExpectFetchReceipts(epoch.Hash, l1Info, nil, nil)
		attrBuilder := NewFetchingAttributesBuilder(cfg, l1Fetcher, l1CfgFetcher, electionClient)
		attrs, err := attrBuilder.PreparePayloadAttributes(context.Background(), l2Parent, epoch)
		require.NoError(t, err)
		require.NotNil(t, attrs)
		require.Equal(t, l2Parent.Time+cfg.BlockTime, uint64(attrs.Timestamp))
	...
	t.Run("new origin with deposits on post-Isthmus", func(t *testing.T) {
		...
		seqNumber := uint64(0)
		epoch := l1Info.ID()
		l1InfoTx, err := L1InfoDepositBytes(cfg, testSysCfg, seqNumber, l1Info, 0, common.Address{})
		require.NoError(t, err)
	...
	t.Run("regolith", func(t *testing.T) {
		...
		for _, tc := range testCases {
			t.Run(tc.name, func(t *testing.T) {
				...
				l1InfoTx, err := L1InfoDepositBytes(cfg, testSysCfg, 0, l1Info, time, common.Address{})
				require.NoError(t, err)
				l1Fetcher.ExpectFetchReceipts(epoch.Hash, l1Info, nil, nil)
				attrBuilder := NewFetchingAttributesBuilder(cfg, l1Fetcher, l1CfgFetcher, emptyElectionClient)
				attrs, err := attrBuilder.PreparePayloadAttributes(context.Background(), l2Parent, epoch)
