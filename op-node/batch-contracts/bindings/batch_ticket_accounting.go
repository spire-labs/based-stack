...
var BatchTicketAccountingMetaData = &bind.MetaData{
	ABI: "[{\"type\":\"constructor\",\"inputs\":[{\"name\":\"_lookaheadValidators\",\"type\":\"address[]\",\"internalType\":\"address[]\"}],\"stateMutability\":\"nonpayable\"}]",
...
func DeployBatchTicketAccounting(auth *bind.TransactOpts, backend bind.ContractBackend, _lookaheadValidators []common.Address) (common.Address, *types.Transaction, *BatchTicketAccounting, error) {
	...
	address, tx, contract, err := bind.DeployContract(auth, *parsed, common.FromHex(BatchTicketAccountingBin), backend, _lookaheadValidators)
	if err != nil {
		return common.Address{}, nil, nil, err
	}
