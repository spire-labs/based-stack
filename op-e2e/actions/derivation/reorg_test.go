...
func RestartOpGeth(gt *testing.T, deltaTimeOffset *hexutil.Uint64) {
	...
	require.NoError(t, err)
	sequencer := actionsHelpers.NewL2Sequencer(t, log, l1F, miner.BlobStore(), miner.BeaconClient(), altda.Disabled, l2Cl, sd.RollupCfg, 0, nil)
...
func ConflictingL2Blocks(gt *testing.T, deltaTimeOffset *hexutil.Uint64) {
	...
	l1F, err := sources.NewL1Client(miner.RPCClient(), log, nil, sources.L1ClientDefaultConfig(sd.RollupCfg, false, sources.RPCKindStandard))
	require.NoError(t, err)
	altSequencer := actionsHelpers.NewL2Sequencer(t, log, l1F, miner.BlobStore(), miner.BeaconClient(), altda.Disabled, altSeqEngCl, sd.RollupCfg, 0, nil)
